      ...
      <div class="sign-content">
        ...
        >
          ...
          >
            ...
            <el-form-item><div @click="back">点我返回</div></el-form-item>
          </el-form>
        </el-main>
      </div>
    </el-container>
...
this.$router.push('/main')} , 
    apiPost () {
      this.$http
        .post('/api/v1/operator/login', { "email": this.SigninForm.email, "password": this.SigninForm.pass })
        .then(result => {
          if (result.data == "0") {
            this.$message({
              message: '登录成功!',
            ...
            this.$http
              .post('/api/v1/operator/getId', {                "email": this.SigninForm.email,
                "password": this.SigninForm.pass             })
              .then(result => {
                this.operatorId = result.data
                this.$router.push({ path: `/smain/${this.operatorId}` })
              })
