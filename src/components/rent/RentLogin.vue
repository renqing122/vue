...
export default {
  ...
  methods: {
    ...
    apiPost () {
      this.$http
        .post('/api/v1/tenant/login', { "email": this.SigninForm.email, "password": this.SigninForm.pass })
        .then(result => {
          if (result.data == "0") {
            this.$message({
              message: '登录成功!',
            ...
            this.$http
              .post('/api/v1/tenant/queryTenantIdByEmail', { "email": this.SigninForm.email })
              .then(result => {
                this.tenantId = result.data
                this.$router.push({ path: `/rmain/${this.tenantId}` })
              })
          } else if (result.data == "1") {
            this.$message.error('您的账号或密码错误！');
          } else {
            this.$message.error('该邮箱未注册或未激活！');
          }
        })
    },
    Signin (formName) {
    ...
    send () {
      this.$http
        .post('/api/v1/tenant/forgetPassword', { "email": this.formLabelAlign.email })
        .then(result => {
          if (result.data == "1") {
